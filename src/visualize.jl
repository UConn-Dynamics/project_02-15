...
function plot_positions(time, q_all; filename="results/positions_vs_time.png")
    ...
    # ----- Panel 1: Piston 1 -----
    p1 = plot(time, q_all[:, 1], label=L"x_1", lw=2, color=:blue, legend=:topleft)
    plot!(p1, time, q_all[:, 2], label=L"y_1", lw=2, color=:orange)
    ylabel!(p1, "Position (m)")
    p1r = twinx(p1)
    plot!(p1r, time, q_all[:, 3], label=L"\theta_1", lw=2, ls=:dash, color=:green, legend=:topright)
    ...
    title!(p1, "Piston 1 Coordinates")
     ...
     # ----- Panel 2: Piston 2 -----
    p2 = plot(time, q_all[:, 4], label=L"x_2", lw=2, color=:blue, legend=:topleft)
    plot!(p2, time, q_all[:, 5], label=L"y_2", lw=2, color=:orange)
    ylabel!(p2, "Position (m)")
    p2r = twinx(p2)
    plot!(p2r, time, q_all[:, 6], label=L"\theta_2", lw=2, ls=:dash, color=:green, legend=:topright)
    ...
    title!(p2, "Piston 2 Coordinates")
     ...
     # ----- Panel 3: Bar ----------
    p3 = plot(time, q_all[:, 7], label=L"x_3", lw=2, color=:blue, legend=:topleft)
    plot!(p3, time, q_all[:, 8], label=L"y_3", lw=2, color=:orange)
    ylabel!(p3, "Position (m)")
    p3r = twinx(p3)
    plot!(p3r, time, q_all[:, 9], label=L"\theta_3", lw=2, ls=:dash, color=:green, legend=:topright)
...
function plot_velocities(time, dq_all; filename="results/velocities_vs_time.png")
    ...
    # ----- Panel 1: Piston 1 -----
    p1 = plot(time, dq_all[:, 1], label=L"\dot{x}_1", lw=2, color=:blue, legend=:topleft)
    plot!(p1, time, dq_all[:, 2], label=L"\dot{y}_1", lw=2, color=:orange)
    ylabel!(p1, "Velocity (m/s)")
    p1r = twinx(p1)
    plot!(p1r, time, dq_all[:, 3], label=L"\dot{\theta}_1", lw=2, ls=:dash, color=:green, legend=:topright)
    ...
    title!(p1, "Piston 1 Velocities")
     ...
     # ----- Panel 2: Piston 2 -----
    p2 = plot(time, dq_all[:, 4], label=L"\dot{x}_2", lw=2, color=:blue, legend=:topleft)
    plot!(p2, time, dq_all[:, 5], label=L"\dot{y}_2", lw=2, color=:orange)
    ylabel!(p2, "Velocity (m/s)")
    p2r = twinx(p2)
    plot!(p2r, time, dq_all[:, 6], label=L"\dot{\theta}_2", lw=2, ls=:dash, color=:green, legend=:topright)
    ...
    title!(p2, "Piston 2 Velocities")
     ...
     # ----- Panel 3: Bar ----------
    p3 = plot(time, dq_all[:, 7], label=L"\dot{x}_3", lw=2, color=:blue, legend=:topleft)
    plot!(p3, time, dq_all[:, 8], label=L"\dot{y}_3", lw=2, color=:orange)
    ylabel!(p3, "Velocity (m/s)")
    p3r = twinx(p3)
    plot!(p3r, time, dq_all[:, 9], label=L"\dot{\theta}_3", lw=2, ls=:dash, color=:green, legend=:topright)
...
function plot_accelerations(time, ddq_all; filename="results/accelerations_vs_time.png")
    ...
    # ----- Panel 1: Piston 1 -----
    p1 = plot(time, ddq_all[:, 1], label=L"\ddot{x}_1", lw=2, color=:blue, legend=:topleft)
    plot!(p1, time, ddq_all[:, 2], label=L"\ddot{y}_1", lw=2, color=:orange)
    ylabel!(p1, "Acceleration (m/s^2)")
    p1r = twinx(p1)
    plot!(p1r, time, ddq_all[:, 3], label=L"\ddot{\theta}_1", lw=2, ls=:dash, color=:green, legend=:topright)
    ...
    title!(p1, "Piston 1 Accelerations")
     ...
     # ----- Panel 2: Piston 2 -----
    p2 = plot(time, ddq_all[:, 4], label=L"\ddot{x}_2", lw=2, color=:blue, legend=:topleft)
    plot!(p2, time, ddq_all[:, 5], label=L"\ddot{y}_2", lw=2, color=:orange)
    ylabel!(p2, "Acceleration (m/s^2)")
    p2r = twinx(p2)
    plot!(p2r, time, ddq_all[:, 6], label=L"\ddot{\theta}_2", lw=2, ls=:dash, color=:green, legend=:topright)
    ...
    title!(p2, "Piston 2 Accelerations")
     ...
     # ----- Panel 3: Bar ----------
    p3 = plot(time, ddq_all[:, 7], label=L"\ddot{x}_3", lw=2, color=:blue, legend=:topleft)
    plot!(p3, time, ddq_all[:, 8], label=L"\ddot{y}_3", lw=2, color=:orange)
    ylabel!(p3, "Acceleration (m/s^2)")
    p3r = twinx(p3)
    plot!(p3r, time, ddq_all[:, 9], label=L"\ddot{\theta}_3", lw=2, ls=:dash, color=:green, legend=:topright)
    theta_3_val = ddq_all[1, 9]          # pull constant value of theta
    ylims!(p3r, (theta_3_val - theta_pad, theta_3_val + theta_pad))
    ylabel!(p3r, "Angular Acceleration (rad/s²)")
...
function animate_dashboard(time, q_all, dq_all, ddq_all; filename="results/dashboard.gif", fps=30)
    ...
    anim = @animate for i in 1:N
        ...
        p2 = plot(time[1:i], q_all[1:i, 1], label=L"x_1", lw=2, color=:orange, xlabel="t (s)", ylabel="Position (m)", title="Piston Positions",
                  xlim=(time[1], time[end]))
        
        plot!(p2, time[1:i], q_all[1:i, 4], label=L"x_2", lw=2, color=:green)
        ...
        p3 = plot(time[1:i], dq_all[1:i, 1], label=L"\dot{x}_1", lw=2, color=:orange, xlabel="t (s)", ylabel="Velocity (m/s)", title="Piston Velocities",
                  xlim=(time[1], time[end]))
        
        plot!(p3, time[1:i], dq_all[1:i, 4], label=L"\dot{x}_2", lw=2, color=:green)
        ...
        p4 = plot(time[1:i], ddq_all[1:i, 1], label=L"\ddot{x}_1", lw=2, color=:orange, xlabel="t (s)", ylabel="Acceleration (m/s^2)", title="Piston Accelerations",
                  xlim=(time[1], time[end]))
        
        plot!(p4, time[1:i], ddq_all[1:i, 4], label=L"\ddot{x}_2", lw=2, color=:green)
